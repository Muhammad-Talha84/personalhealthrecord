...
const PreBloodCP = () => {
    ...
    try {
      const stmt = db.prepare(
        `SELECT parameter, result AS value, unit, date, time, minValue, maxValue
         FROM LabReports
         WHERE profileName = ? AND testName = 'BloodCP'`
      );
      ...
      console.error("Error fetching Blood CP entries:", error);
    }
  }, [db, profile]);
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>Blood CP Test History</h2>
      ...
      {entries.length > 0 ? (
        ...
        <p style={styles.noData}>No Blood CP records found.</p>
      )}

      <button
        style={styles.button}
        onClick={() => navigate("/cp", { state: { profile } })}
      >
        Add New Blood CP Entry
      </button>
    </div>
  );
};

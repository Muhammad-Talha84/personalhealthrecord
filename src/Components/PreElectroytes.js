...
const PreElectroytes = () => {
  ...
  useEffect(() => {
    ...
    const stmt = db.prepare(
      `SELECT parameter, result AS value, unit, date, time, minValue, maxValue
       FROM LabReports
       WHERE profileName = ? AND testName = 'Electrolytes'`
    );
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>Electrolytes Test History</h2>
      ...
      {entries.length > 0 ? (
        ...
        <p style={styles.noData}>No Electrolytes records found.</p>
      )}

      <button
        style={styles.button}
        onClick={() => navigate("/electrolytes", { state: { profile } })}
      >
        Add New Electrolytes Entry
      </button>
    </div>
  );
};

...
const PreRFT = () => {
    ...
    try {
      const stmt = db.prepare(
        `SELECT parameter, result AS value, unit, date, time, minValue, maxValue
         FROM LabReports
         WHERE profileName = ? AND testName = 'RFT'`
      );
      ...
      console.error("Error fetching RFT entries:", error);
    }
  }, [db, profile]);
  ...
  const handleClick = (entry) => {
    ...
    navigate("/reportdetail", {
      state: { profile, type: "RFT", readings },
    });
  };

  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>Renal Function Test History</h2>
      ...
      {entries.length > 0 ? (
        ...
        <p style={styles.noData}>No RFT records found.</p>
      )}

      <button
        style={styles.button}
        onClick={() => navigate("/rft", { state: { profile } })}
      >
        Add New RFT Entry
      </button>
    </div>

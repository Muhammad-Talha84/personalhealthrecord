...
const PreLFT = () => {
    ...
    try {
      const stmt = db.prepare(
        `SELECT parameter, result AS value, unit, date, time, minValue, maxValue
         FROM LabReports
         WHERE profileName = ? AND testName = 'LFT'`
      );
      ...
      console.error("Error fetching LFT entries:", error);
    }
  }, [db, profile]);
  ...
  const handleClick = (entry) => {
    ...
    navigate("/reportsdetail", {
      state: { profile, type: "LFT", readings },
    });
  };

  return (
    <div style={styles.container}>
      <h2 style={styles.heading}>Liver Function Test History</h2>
      ...
      {entries.length > 0 ? (
        ...
        <p style={styles.noData}>No LFT records found.</p>
      )}

      <button
        style={styles.button}
        onClick={() => navigate("/lft", { state: { profile } })}
      >
        Add New LFT Entry
      </button>
    </div>
